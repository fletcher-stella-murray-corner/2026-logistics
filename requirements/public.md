...
- **Plain actions** (items 3, 4, 5) — a jump ("Now"), a toggle ("▶"/"⏸"), or a link to another page ("Tree"): no caret, since none of them expand anything in place. All three, plus both disclosure triggers, share one hover rule — underline on hover — with exactly one documented exception: the icon-only play/pause toggle dims instead, since underlining a lone glyph reads oddly. That's the only place an item's hover differs, and it differs because of what it visually *is* (an icon vs. text), not by unstated choice.
