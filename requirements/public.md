...
**Timeline page** — nav bar is a single sticky row, always visible, six items left to right. Six items, but one small visual system, not six independent ones — every item's look follows from which of three roles it plays, and nothing about an item's style is picked ad hoc:
- **The one fixed identity** (item 1) — bold, no caret, never changes.
- **Disclosure triggers** (items 2 and 6) — text plus a caret ("▾"), always the exact same caret component (markup, size, spacing — see item 2), since a caret's one job here is "tapping this expands a panel in place," and that has to look identical everywhere it's true.
- **Plain actions** (items 3, 4, 5) — a jump ("Now"), a toggle ("▶"/"⏸"), or a link to another page ("Tree"): no caret, since none of them expand anything in place. All three, plus both disclosure triggers and every other nav link across every page (including the Family Tree and Attendees pages' own two-item navs — see below), are **permanently underlined**, not underlined-on-hover — deliberate, since hover doesn't exist on a touch device and most family will be checking this from a phone (see `brand-guidelines.md` → *Audience*); a hover-only cue would leave a phone visitor with no way to tell a nav item is tappable until they happened to tap it. Exactly one documented exception: the icon-only play/pause toggle dims on hover instead of gaining an underline, since underlining a lone glyph reads oddly rather than as "this is clickable text." That's the only place an item's interactive-state styling differs, and it differs because of what it visually *is* (an icon vs. text), not by unstated choice.

Weight and size carry hierarchy within all of this (the site title's extra boldness, the live label's date-vs-day/quarter emphasis) — see `brand-guidelines.md` → *Colours* for why color is never used for that job instead. New nav items should slot into one of these three roles rather than inventing a fourth look.

1. **"Murray Corner 2026"** — the site's one permanent, always-present way back to the Timeline, but what it actually does depends on where you click it from, since "back to the Timeline" means something different on the Timeline itself than it does everywhere else. Unlike every other item in this nav, its content never changes as you scroll — it's the one fixed point in the row. On the Timeline, it's an in-page anchor to the very top (the intro screen) — a link to somewhere *above* "now", never *to* "now" itself, since you're already on the Timeline. On the Family Tree and every Attendees page, it's instead a plain link to `../index.html` with no `#qc-...` hash — a full navigation, not an anchor — which means it lands on **"now"**, not the intro screen, purely as a consequence of the Timeline's own hash-less-load behavior (see *Always the full trip, "now" computed live* below), not anything this nav item does specially. This is deliberate: from off the Timeline, "now" is the more useful landing spot (what's happening at the cottage right now), while the intro screen stays one scroll away like any other point earlier in the trip.
2. **Current day quarter label, doubling as jump-to-time** — live text showing which day quarter is currently in view, updated as you scroll (via `IntersectionObserver` in `timeline/shared.js`), in two parts, in this order: the bare month/day (e.g. "Aug 3"), shown smaller and in the secondary color as supporting detail; then the weekday name in full plus the bare quarter name, joined by " · " (e.g. "· Monday Morning") — the same middot separator used everywhere else on the site two related pieces of info sit together (`QUARTER_LABELS`'s own "Morning · 6am–12pm", the Attendees page's "Aug 1 · 6pm"), not a hyphen or any other punctuation invented just for this one label — shown large/bold/prominent since that's what you actually scan for while scrolling — no time-range suffix here (unlike the jump-to-time dropdown's own link text, e.g. "Morning · 6am–12pm", which keeps it) — plus a small "▾" indicating it's also a disclosure trigger, the exact same caret (same markup, size, and spacing) the "Folks ▾" disclosure below uses — one shared visual component for "this expands," not two different ones that happen to use the same glyph. **`00-06` is the one exception to the "weekday plus quarter name" shape**: it has no quarter name (see *Terminology* above), so the label there is just the weekday alone, e.g. "· Tuesday" — not "· Tuesday Night." Empty (no text) before you've scrolled into any quarter screen, and is the only place this date/time information appears (see *Terminology* — it's deliberately not repeated on the canvas). Clicking/tapping the label itself — not a separate "Jump ▾" control next to it — expands a dropdown disclosure listing every day, each with its full set of day-quarter times as links, `00-06`'s own link reading just its bare time range ("12am–6am") for the same reason. Clicking one does a smooth animated scroll to that quarter screen (`scrollIntoView({behavior: 'smooth'})` in `timeline/shared.js`, not the CSS `scroll-behavior` property — see that file for why) and closes the disclosure. This is the reliable way to reach any quarter screen; don't assume scroll/swipe alone is enough.
...
**Family Tree page** — nav bar is `Murray Corner 2026` link, `Tree` active/current-page indicator, then its own "Folks ▾" — same disclosure shape and caret as the Timeline's (`family-tree/scripts/build.py` → `render_folks_menu()`), but a different list and a different reason for existing: every *attending* person (from `shared/data/people.json` directly, not `travel.json` — see below for why that distinction matters), alphabetically by name, each entry a name plus a single "Detail" link straight to their Attendees page, with an extra "Collecting facts" segment for anyone still in that state (see *Family Tree* → *Layout* below) — omitted entirely for the default/common case (facts already collected), same "no extra treatment for the expected case" convention the tree boxes themselves use. Not-attending people are left out, same as the Attendees feature itself. This is where the words "Collecting facts" now live for this page — see *Family Tree* → *Layout* below for why they no longer sit directly on the tree. Unlike the Timeline's own "Folks ▾" (built from `travel.json` entries, so someone with no entry yet wouldn't appear at all), this one has to be built from `people.json` directly, since the whole point is surfacing people who *don't* have an entry yet. Still no live label, "Now", or Run — those stay Timeline-only, since only the Timeline has quarter screens to label, jump between, jump to "now" within, or auto-advance through.
...
No index page and no nav link of its own (see *Navigation* above) — reached three ways: clicking an attending person's box on the Family Tree, the "Detail" link next to their name in the Timeline's "Folks ▾" disclosure, or the "Detail" link in the Family Tree's own "Folks ▾" disclosure (see *Navigation* above). Organizers checking who's still owed a "what are your plans" conversation use that Family Tree "Folks ▾" dropdown for that now — every attending person listed with a "Collecting facts" segment for anyone still in that state — rather than a dedicated grouped list of their own.
...
**`site/attendees/<id>.html`** (one per person with `attending: true`, `<id>` is their stable `shared/data/people.json` id, e.g. `site/attendees/7.html` for David) — nav bar per *Navigation* above (the plain two-item `Murray Corner 2026`/`Tree` row, `Murray Corner 2026` landing on "now" back on the Timeline, `Tree` being the way back to wherever you clicked through from). Below it, the person's name as a heading, then:
...
- Otherwise, a **single chronological timeline grouped under one day heading per distinct date** (see `attendees/scripts/build.py`'s `render_person_page()`) — not a flat Arrival/Departure/Room breakdown and not one block per kind of fact either. Every fact below (Arrival, Sleeping, Working from, Departure, Driving) is collected first and sorted together by date, exactly as before, so a Driving obligation or a Working from stretch that falls between two Sleeping stretches still reads in its actual place in the trip rather than trailing after a much-later Departure — the only change is that consecutive facts sharing the exact same date are now nested under one **day heading** (bold, Primary color, e.g. "Tuesday, Aug 4" — see `brand-guidelines.md` → *Typography* → *Headlines*, which specifies "Day headings" as their own type-level element) instead of each repeating its own full date. A **ranged** fact (Sleeping, Working from) is grouped under the *first* date of its range — the fact line itself still states the full range, only which day heading it sits under is decided by where it starts. When two facts land on the exact same date, ties under that one heading keep a sensible default order: Arrival, then Sleeping, then Working from, then Departure, then Driving.
  ...
  - **Working from** — no leading time. Detail: the structure, then its quarters (e.g. "mornings & afternoons"), then a `", till <end day>"` clause when the block spans more than one day (omitted for a single-day block) — e.g. "Working from Red Shed, mornings & afternoons till Thursday, Aug 6". One line per `working_from` block (see *Homepage = Timeline* → *Data* → `working_from` above), grouped under each block's own start date — not merged or re-split like the Sleeping milestones above, since a block is already exactly the range the editor intended.
