...
Every split control and every plain link is **permanently underlined**, not underlined-on-hover — deliberate, since hover doesn't exist on a touch device and most family will be checking this from a phone (see `brand-guidelines.md` → *Audience*); a hover-only cue would leave a phone visitor with no way to tell a nav item is tappable until they happened to tap it. The caret on a split control is the exact same shared component (markup, size, spacing) everywhere it appears, so "there's more to pick here" always looks identical — except while the Timeline panel's own auto-advance is running, the one deliberate exception (see *Timeline's panel* below).

### Timeline's panel

Opened by the caret next to the "Timeline" nav item (see above) — the disclosure this project used to call "the current day quarter label." Its live-label behavior is unchanged: while scrolled into a quarter screen, the item's text switches from the static word "Timeline" to two live parts, updated as you scroll (via `IntersectionObserver` in `timeline/shared.js`) — the bare month/day (e.g. "Aug 3"), smaller/secondary, then the weekday name plus bare quarter name joined by " · " (e.g. "· Monday Morning"), large/bold/prominent, `00-06` showing just the weekday alone per *Terminology* above. Reverts to the static "Timeline" text on every other view, where there's no current quarter to report. Inside the panel:
...
The list a Folks caret opens has always had two different sources depending on which page you're on — that part is unchanged by this redesign, only now reachable identically (a caret next to a split control) everywhere instead of a bespoke "Folks ▾" per page:

- **On Home/Timeline** (`timeline/scripts/build.py` → `render_folks_menu()`): built from `timeline/data/travel.json`, so someone with no travel entry yet isn't on the trip and isn't listed. Each entry shows the person's **name** as plain text, followed by two labeled links: **"Timeline"** scrolls to the quarter screen where they first appear (their arrival's day+quarter, or the very first quarter screen of the trip if they have no `arrival` at all); **"Detail"** goes straight to their Details page, leaving the Timeline entirely. Omitted entirely if `travel.json` is empty.
- **On Family Tree and Details pages** (`family-tree/scripts/build.py` → `render_folks_menu()`): a different list and a different reason for existing — built from `people.json` directly instead, since the whole point here is surfacing people who *don't* have a travel entry yet. Every *attending* person, alphabetically by name, each entry a name plus a single "Detail" link, with an extra "Collecting facts" segment for anyone still in that state (see *Family Tree* → *Layout* below) — omitted entirely for the default/common case (facts already collected). Not-attending people are left out, same as the Attendees feature itself. Details pages didn't have a Folks item of their own before this redesign; now that they do, they reuse this variant rather than the travel.json one, since a Details page has no quarter screens to jump a person's "Timeline" link to in the first place.
...
Nav bar per *Navigation* above. Below it, people grouped by `generation` (from `shared/data/people.json`), lowest number first. Within a generation, partners (`partner_id`) are shown paired together; each person/couple's children (people whose `parent_ids` includes them) are shown nested/indented below, left to right in `birth_order` order (see *Data* → `birth_order` above — oldest first, falling back to `id` for anyone without one set) — e.g. an older sibling added to the file after a younger one already there (a higher `id`) still needs to render first, which is exactly what `birth_order` is for. The same ordering rule applies to the top-level roots themselves when there's more than one family line at the root.

**No text on the tree itself says "Married in," "Collecting facts," or "Not attending" — those three states are communicated visually only** (border style/color, a background tint, opacity), never spelled out under a name the way earlier versions of this page did. The words themselves live one tap away, in the nav's own "Folks ▾" dropdown (see *Navigation* above) — a person's own Attendees page also states their status directly where relevant. This trades a caption everyone can read at a glance for a visual language that stays out of the way once you've learned it once; the tree is meant to be scanned as a shape (who's connected to whom), not read line by line, and a caption under every third name worked against that. Within the site's plain/no-icon visual language (`brand-guidelines.md`), no icons are used for any of this either — only border style/color, a background tint, and opacity.
...
No index page of its own — reached by clicking an attending person's box on the Family Tree, the "Detail" link next to their name in a Folks panel (from any page — see *Navigation* above), or the "Folks" nav item's own direct click, which lands on a random attending person rather than a specific one. Organizers checking who's still owed a "what are your plans" conversation use the people.json-sourced Folks panel (Family Tree and Details pages — see *Navigation* → *Folks panel* above) for that — every attending person listed with a "Collecting facts" segment for anyone still in that state — rather than a dedicated grouped list of their own.
