...
Four items, left to right, the same set on every nav bar, every view:
...
4. **Tree** — a plain link to the Family Tree page. No caret, no secondary behavior — the one plain-action item, alongside the fixed identity and the two split controls.

No underline on any nav item — bold weight, color, and this row's own fixed, always-in-the-same-place position at the top of every page are the tappable cue instead, a short enough, same-every-time set of items (MC26, Timeline, Folks, Tree) that it reads as "the nav" on sight. The caret on a split control is the exact same shared component (markup, size, spacing) everywhere it appears, so "there's more to pick here" always looks identical — except while the Timeline panel's own auto-advance is running, the one deliberate exception (see *Timeline's panel* below).

### Timeline's panel

Opened by the caret next to the "Timeline" nav item (see above) — the disclosure this project used to call "the current day quarter label." Its live-label behavior is unchanged: while scrolled into a quarter screen, the item's text switches from the static word "Timeline" to two live parts, updated as you scroll (via `IntersectionObserver` in `timeline/shared.js`) — the abbreviated, weekday-first date (e.g. "Wed, Aug 5" — see `shared/trip.py` -> `format_date_abbrev()`), smaller/secondary, then the bare quarter name (e.g. "· Morning"), large/bold/prominent, `00-06` showing just the weekday+date alone with no quarter suffix per *Terminology* above. Kept abbreviated deliberately — this is a single-row control sharing space with three other nav items on a phone, so it doesn't use the fuller "Wednesday, August 5th" phrasing the Attendees page and this same panel's own day headings use below, where there's room for it. Reverts to the static "Timeline" text on every other view, where there's no current quarter to report. Inside the panel:

- **The day/quarter jump list** — every day, each with its full set of day-quarter times as links, `00-06`'s own link reading just its bare time range ("12am–6am"). Unchanged from before, and present in this panel on every view (Family Tree, Details pages included) — a link straight to a specific quarter is just as useful from off `site/index.html` as on it. No separate "Now" entry here — the Timeline nav item's own label already covers that exact case in one tap, without opening anything.
...
Clicking any link in this panel closes the disclosure after acting, same as before.
...
Clicking any entry's link closes the disclosure after acting, same as before.
...
- `birth_order` — optional integer, defaults to that person's own `id` when omitted. Used only by the Family Tree page, to decide top-to-bottom display order among siblings (children of the same parent(s)) and among the top-level roots — lower sorts first (oldest first). This is the one field that exists purely to override display order, because `id` itself can't be reused for that: ids are permanent and never renumbered (see `id` above), but people are usually entered in whatever order the editor learns about them, not birth order — so a newly-added older sibling almost always ends up with a *higher* id than younger siblings already in the file. Set `birth_order` only on the sibling(s) that need to sort out of id order (typically just the one being inserted); everyone else keeps the default (their own id), which is why the common case — siblings already entered oldest-to-youngest — needs no `birth_order` at all. Only meaningful relative to the other people it's actually compared against (the same sibling group, or the other roots) — there's no global numbering scheme across the whole tree. Validated at build time: must be an integer when present.
...
**No text under any individual name says "Married in," "Collecting facts," or "Not attending" — those three states are communicated visually only** (border style/color, a background tint, opacity), never spelled out under a name the way earlier versions of this page did; the tree is meant to be scanned as a shape (who's connected to whom), not read line by line, and a caption under every third name worked against that. Instead, a small legend directly below the page subtitle spells out what each visual treatment means, once, in one place — real (empty) `.person` boxes wearing the same modifier classes as the tree itself, so it can't drift out of sync with what the boxes actually look like (see `family-tree/scripts/build.py` → `render_legend()`). A person's own Attendees page also states their status directly where relevant — the nav's own "Folks ▾" dropdown no longer does (see *Navigation* → *Folks panel* above): it's a jump list now, not a status report. Within the site's plain/no-icon visual language (`brand-guidelines.md`), no icons are used for any of this either — only border style/color, a background tint, and opacity.
...
**`site/attendees/<id>.html`** (one per person with `attending: true`, `<id>` is their stable `shared/data/people.json` id, e.g. `site/attendees/7.html` for David) — nav bar per *Navigation* above (the same four items as every other page: MC26, Timeline, Folks, Tree). Below it, the person's name as a heading, then:
...
- Otherwise, a **single chronological timeline grouped under one day heading per distinct date** (see `attendees/scripts/build.py`'s `render_person_page()`) — not a flat Arrival/Departure/Room breakdown and not one block per kind of fact either. Every fact below (Arrival, Sleeping, Working from, Departure, Driving) is collected first and sorted together by date, exactly as before, so a Driving obligation or a Working from stretch that falls between two Sleeping stretches still reads in its actual place in the trip rather than trailing after a much-later Departure — the only change is that consecutive facts sharing the exact same date are now nested under one **day heading** (bold, Primary color, e.g. "Tuesday, Aug 4" — see `brand-guidelines.md` → *Typography* → *Headlines*, which specifies "Day headings" as their own type-level element) instead of each repeating its own full date. A **ranged** fact (Sleeping, Working from) is grouped under the *first* date of its range — the fact line itself still states the full range, only which day heading it sits under is decided by where it starts. When two facts land on the exact same date, ties under that one heading keep a sensible default order: Arrival, then Sleeping, then Working from, then Departure, then Driving.
  - **Every fact renders as one flowing line**, not a three-column row: for a **point-in-time** fact (Arrival, Departure, Driving) — a leading **time** (bold, Primary — the `time_range` if one is set, e.g. "5:15pm", otherwise the bare day quarter name, e.g. "Afternoon" — `00-06` instead shows just its bare time range, "12am–6am", no name, same rule as the nav's live label — see *Terminology*), then a short **label** (secondary color, e.g. "Arrival", "Departure", "Driving"), then the **detail**. A **date-range** fact (Sleeping, Working from) has no single time of day to lead with, so its line just starts at the label. Every date on this page — the day heading and any inline "till..." mention alike — is the full natural-speech form: full weekday, full month, ordinal day (e.g. "Wednesday, August 5th," not "Wed, Aug 5" or "Aug 5") — see `shared/trip.py` -> `format_date_full()`. This page has the room for the fuller phrasing; the Timeline's own live nav label doesn't, and stays abbreviated instead (see *Navigation* -> *Timeline's panel* above). A room reference always keeps its full stored name, em-dash included (e.g. "Cottage — Master Suite"), matching how it's written in `travel.json` and shown everywhere else on the site (the Timeline's own Structures row).
  - **Arrival** — time: as above. Detail: `"by <mode tag+name>"` (e.g. "by 🚗 Car"), then hub/vehicle/free-text detail/driver, each appended after an em-dash — e.g. "5:15pm **Arrival** by 🚗 Car — White Toyota Camry". If `arrival` is omitted: no leading time, detail reads "Already at the accommodation" (grouped under an August 1 day heading, sorting as if dated August 1).
  - **Sleeping** — no leading time. Detail: the room, plus a `" till <end day>"` clause when the stretch spans more than one day (omitted for a single-day stretch — e.g. Susan's first night in the Master Suite reads just "Sleeping Cottage — Master Suite", with no "till" clause, since it's a single night) — e.g. "Sleeping Cottage — Blue Room till Tuesday, Aug 11". One line per contiguous same-room date range, in date order. Computed by walking every date from arrival (or August 1 if omitted) through the last date they're actually present for (the day before departure, if departure's quarter is `00-06` — the very first quarter of that date, meaning they're gone before any of it — otherwise the departure date itself; or August 15 if departure is omitted) and collapsing consecutive same-room dates into one range — the same `room`/`room_by_date` resolution and the same presence rule the Timeline's Structures row uses, just grouped into ranges instead of listed per night, and re-split into a new milestone if the same room recurs non-consecutively (e.g. Cottage → Tent → Cottage again shows as three separate Sleeping lines, not two, since the two Cottage stretches aren't adjacent). Any date range covered by an `excursion` (see *Data* → `excursions` above) is skipped in this walk entirely — the stay splits into two Sleeping milestones with a gap between them (and a day heading of its own for whichever date the stay resumes on), rather than one range that wrongly includes time the person was away. This page keeps the "Sleeping" label even though the Timeline's equivalent row is called "Structures" — here it's specifically framed as this person's own overnight story, not a shared live view of every structure, so the more personal label still fits.
  - **Working from** — no leading time. Detail: the structure, then its quarters (e.g. "mornings & afternoons"), then a `", till <end day>"` clause when the block spans more than one day (omitted for a single-day block) — e.g. "Working from Red Shed, mornings & afternoons till Thursday, Aug 6". One line per `working_from` block (see *Home & Timeline* → *Data* → `working_from` above), grouped under each block's own start date — not merged or re-split like the Sleeping milestones above, since a block is already exactly the range the editor intended.
