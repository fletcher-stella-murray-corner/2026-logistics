...
| 9 | [shared/data/structures.json](shared/data/structures.json) | The fixed list of named accommodation/transit locations (cottage, red shed, airports, etc), each optionally with a fixed always-shown `rooms` list in display order (e.g. Cottage), a `buildings` list grouping fixed rooms under more than one physical building at the same site (e.g. Little Shemogue Inn's Log Point/The Apartment — mutually exclusive with `rooms`), a validated occupancy-shown `instances` list (e.g. The Field — mutually exclusive with `rooms`/`buildings`), an `always_shown` flag for a structure with no fixed rooms that should still always render (e.g. Red Shed), a `spaces` list declaring non-sleeping activity kinds it supports (`kitchen`/`working`), and an `active_from`/`active_to` date range. Validated against — not just referenced by — `timeline/data/travel.json`'s `room`/`room_by_date`/`room_by_quarter`/`hub`/`working_from` fields and `timeline/data/meals.json`'s `structure` field at build time (see `requirements/public.md` → *Structures*). |
