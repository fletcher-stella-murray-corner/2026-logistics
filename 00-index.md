...
| 5 | [requirements/public.md](requirements/public.md) | What the public-facing site should be/do — the Timeline (homepage) and Family Tree features. Source of truth for desired behavior on the site. There is no admin site or admin requirements doc for this project. |
...
A feature's own requirements (what it does, page by page) live in `requirements/public.md`, not in the feature's own folder. Each feature folder holds everything else: scripts, data, and shared CSS. There are two features, plus a `shared/` folder for what both need in common:
