...
- **The root cause behind the above, and a cluster of other browser-preview-tool quirks, is that the pane is frequently `document.visibilityState: "hidden"`** — backgrounded from the real browser's own point of view whenever it isn't the frontmost thing the tool is actively interacting with, which is often, given how many tool calls a single verification pass makes. A hidden tab is throttled by the browser exactly as the Page Visibility API spec says it should be: `requestAnimationFrame` callbacks stop or slow drastically (confirmed directly — a rAF-chained test timed out after 30s with the pane backgrounded), `scrollend` and smooth-scroll animations can stall or never fire, `IntersectionObserver` callbacks can arrive many seconds late relative to the scroll change that should have triggered them (confirmed directly — a callback landed on a stale scroll position several tool-calls after the state that should have produced it), and `computer` screenshots can show a stale/frozen compositor frame even when `window.scrollY`/`getBoundingClientRect()` already report the correct, current position. Before concluding any scroll-driven, rAF-driven, or observer-driven behavior is broken: check `document.visibilityState`/`document.hidden` first — if hidden, the discrepancy is likely this, not the code. **`getBoundingClientRect()`/`getComputedStyle()` stay trustworthy regardless** (real synchronous layout, not throttled the way painting/compositing is) — they're the reliable way to verify scroll-position-dependent state in this tool when visibility can't be guaranteed, screenshots and animation-completion can't.
